...
import org.kiji.schema.shell.{Environment, KijiSystem, AbstractKijiSystem}
...
trait KijiIntegrationTestHelpers {
  ...
  val mNextInstanceId = new AtomicInteger(0);
  ...
  def environment(uri: KijiURI, kijiSystem: AbstractKijiSystem) = {
    new Environment(
      instanceURI=uri,
      printer=System.out,
